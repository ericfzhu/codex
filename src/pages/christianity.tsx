...
export default function ChristianityPage() {
	...
	useEffect(() => {
		loadBibleIndex()
			...
			.catch((err) => {
				console.error('Failed to load Christianity index:', err);
				setError('Christianity embeddings not yet available. Please check back later.');
				setIsLoading(false);
			});
	}, []);
		...
		<main className={`h-screen overflow-hidden flex flex-col bg-white dark:bg-gray-900 ${jetBrainsMono.className}`}>
			...
			<header className="h-10 shrink-0 flex items-center px-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
							...
							history.map((item, index) => (
								<React.Fragment key={item.id}>
									...
									<button
										...
										className={`hover:text-amber-600 ${item.id === currentId ? 'text-amber-600 font-medium' : ''}`}>
										{item.reference}
									</button>
								</React.Fragment>
							))
				...
				<div className="flex items-center gap-4 text-sm shrink-0 ml-4">
					...
					<Link href="/" className="uppercase text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white">
						Back
					</Link>
				</div>
			</header>
					...
					<div className="flex-1 min-h-0 grid grid-cols-3 lg:grid-cols-4 [grid-template-rows:repeat(4,minmax(0,1fr))] lg:[grid-template-rows:repeat(3,minmax(0,1fr))] gap-px bg-gray-200 dark:bg-gray-700">
						{neighbors.slice(0, 12).map((neighbor) => (
							<button
								...
								className="group text-left p-1.5 sm:p-2 lg:p-3 bg-white dark:bg-gray-900 hover:bg-amber-600 hover:text-white dark:hover:bg-amber-600 transition-colors overflow-hidden">
