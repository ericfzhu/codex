...
export default function MormonismPage() {
	...
	useEffect(() => {
		loadMormonIndex()
			...
			.catch((err) => {
				console.error('Failed to load Mormon index:', err);
				setError('Mormonism embeddings not yet available. Please check back later.');
				setIsLoading(false);
			});
	}, []);
	...
	useEffect(() => {
		...
		if (id !== null) {
			...
			router.replace(`/mormonism?id=${random.id}`, undefined, { shallow: true });
		}
	}, [router.isReady, router.query.id, searchIndex, searchForVerse]);

	const handleNeighborClick = (id: number) => {
		router.push(`/mormonism?id=${id}`, undefined, { shallow: true });
	};
		...
		<main className={`h-screen overflow-hidden flex flex-col bg-white dark:bg-gray-900 ${jetBrainsMono.className}`}>
							...
							history.map((item, index) => (
								<React.Fragment key={item.id}>
									...
									<button
										...
										className={`hover:text-blue-600 ${item.id === currentId ? 'text-blue-600 font-medium' : ''}`}>
										{item.reference}
									</button>
								</React.Fragment>
							))
			...
			{isLoading ? (
				...
				<>
					...
					<div className="h-28 shrink-0 p-3 border-b border-gray-200 dark:border-gray-700 bg-blue-600 text-white flex flex-col">
					...
					<div className="flex-1 min-h-0 grid grid-cols-3 lg:grid-cols-4 [grid-template-rows:repeat(4,minmax(0,1fr))] lg:[grid-template-rows:repeat(3,minmax(0,1fr))] gap-px bg-gray-200 dark:bg-gray-700">
						{neighbors.slice(0, 12).map((neighbor) => (
							<button
								...
								className="group text-left p-1.5 sm:p-2 lg:p-3 bg-white dark:bg-gray-900 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-600 transition-colors overflow-hidden">
